...
namespace Khuluma_Server.Hubs
{
    public class ChatHub : Hub
    {
        ...
        public async Task Send(string name, string message, int userid, string groupname, int groupid, bool isFlagged)
        {
            ...
            if (!isFlagged)
            {
                Clients.All.addNewMessageToPage(name, message, timestamp, idVals);
            }
            ...
            chatMessage.timestampString = timestamp.ToString();
            chatMessage.Date = timestamp.ToLongDateString();
            chatMessage.Time = timestamp.ToShortTimeString();
            chatMessage.isFlagged = isFlagged;

            db.ChatMessages.Add(chatMessage);
            db.SaveChanges();

            SendNotificationFromFirebaseCloud(name, message, groupid);
        ...
        public void SendNotificationFromFirebaseCloud(string msgTitle, string messageBody, int groupID)
        {
            try
            {
